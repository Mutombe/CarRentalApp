...
class Rental(models.Model):
    ...
    payment_method = models.CharField(max_length=50, null=False, blank=False, choices=PAYMENT_METHOD_CHOICES, default='USD')
    chauffeur = models.ForeignKey(Chauffeur, on_delete=models.CASCADE, default=None, null=True, blank=True)
    ...
    daily_rental_price = models.DecimalField(max_digits=10, decimal_places=2, default=0.0)
    ...
    def calculate_rental_fee(self):
        ...
        if self.car.ecocash_rate and self.payment_method == 'Ecocash':
            rental_fee = self.car.daily_rental_price * rental_days * self.car.ecocash_rate
            ...
            rental_fee = rental_days * (self.car.daily_rental_price + self.chauffeur.daily_fee)
        else:
...
class RentalHistory(models.Model):
    ...
    customer = models.ForeignKey(CustomUser, on_delete=models.PROTECT, blank=False, default=None)
